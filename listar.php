                ...
                if ($resultado && $resultado !== false) {
                    ...
                    if ($total > 0) {
                        while ($fila = mysqli_fetch_assoc($resultado)) {
                            ...
                            echo "<a href='editar.php?id=" . urlencode($fila['id_paciente']) . "' class='btn-edit'>✏️ Editar</a>";
                            echo "<form method='post' action='eliminar.php' onsubmit=\"return confirm('¿Eliminar paciente?');\" style='display:inline'>";
                            echo "<input type='hidden' name='id' value='" . htmlspecialchars($fila['id_paciente']) . "'>";
                            echo "<button type='submit' class='btn-delete'>🗑️ Eliminar</button>";
                            echo "</form>";
                            echo "</td>";
                            echo "</tr>";
                        }
